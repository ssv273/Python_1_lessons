...
for el in my_list:
    if el > 0:                  # проверка на возможность извлечения корня
        a = math.sqrt(el)       # Вычисляем корень
        if (a//1-a) == 0:       # Проверка на целочисленность
            a=int(a)            # чтобы выводились значения без точки
            new_list.append(a)
...
day_in = input('Введите деньd в формате dd:')
month_in = input("Введите месяц в формате mm:")
...
print('Дата в текстовом формате : {} {} {} года'.format(day[day_in],month[month_in],year))
...
n = int(n)
new_list1 = [random.randint(-100,100) for n in range(0,n) ]
print(new_list1)
